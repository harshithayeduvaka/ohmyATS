...
serve(async (req) => {
  ...
  // Per product rule: unlimited ATS scans without accounts. We only require a Bearer
  // token to be present (Supabase publishable/anon or a user JWT) to keep the endpoint
  // off the open internet — no full JWT verification, which was rejecting legitimate
  // anon/session tokens and surfacing as "Edge Function returned non-2xx status code".
